...
const useOpportunities = (fallback) => {
  const [data, setData] = useState(fallback);
  ...
  useEffect(() => {
    ...
    fetch("/data/opportunities.json?v=" + Date.now())
      .then((res) => (res.ok ? res.json() : null))
      .then((json) => {
        if (mounted && json && Array.isArray(json.items) && json.items.length > 0) {
          setData(json.items);
        }
      })
      .catch((err) => console.warn("Opportunities fetch failed:", err.message));
    return () => { mounted = false; };
  }, []);
