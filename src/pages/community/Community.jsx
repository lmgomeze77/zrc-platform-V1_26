...
//  v2 — with EditionZero sub-view navigation
// ─────────────────────────────────────────────────────────────
...
.ic-rule { width:38px; height:1px; background:#B8982A; margin:38px auto; opacity:0; animation:ic-appear 1s ease forwards 2.4s; }
...
.ic-btn { display:inline-block; font-family:'Cormorant SC',serif; font-size:10.5px; letter-spacing:.38em; color:#B8982A; text-decoration:none; border-bottom:1px solid rgba(184,152,42,.28); padding-bottom:3px; transition:color .4s,border-color .4s; cursor:pointer; background:none; border-top:none; border-left:none; border-right:none; }
.ic-btn:hover { color:#D4B050; border-bottom-color:rgba(212,176,80,.55); }
.ic-btn-ghost { display:inline-block; font-family:'Cormorant SC',serif; font-size:9px; letter-spacing:.38em; color:rgba(184,152,42,.5); text-decoration:none; border-bottom:1px solid rgba(184,152,42,.15); padding-bottom:2px; transition:color .4s,border-color .4s; cursor:pointer; background:none; border-top:none; border-left:none; border-right:none; margin-top:16px; }
.ic-btn-ghost:hover { color:rgba(212,176,80,.8); border-bottom-color:rgba(212,176,80,.35); }
.ic-btn-row { display:flex; flex-direction:column; align-items:center; gap:4px; }
...
export default function Community({ lang = "es" }) {
  const [view, setView] = useState("landing"); // "landing" | "edition-zero"
  ...
  const t = {
    es: {
      ...
      membershipLabel:"Membresía solo por invitación",
      membershipCta:"Solicitar Consideración",
    ...
    en: {
      ...
      membershipLabel:"Membership by invitation only",
      membershipCta:"Request Consideration",
  ...
  // Reset to landing when navigating away and back
        ...
        <div className="ic-stage">
          <div className="ic-center">
            ...
            <div className="ic-access">
              <div className="ic-access-label">{tx.membershipLabel}</div>
              <div className="ic-btn-row">
                <a className="ic-btn" href={"mailto:luis@zenithrisecapital.com?subject=Inner%20Circle%20%E2%80%94%20Request"}>{tx.membershipCta}</a>
                <button className="ic-btn-ghost" onClick={() => setView("edition-zero")}>{tx.readEdition}</button>
              </div>
            </div>
          </div>
        </div>
