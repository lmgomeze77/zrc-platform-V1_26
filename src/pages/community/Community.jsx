...
.ic-grain {
  ...
  animation: ic-grain .8s steps(1) infinite;
}
@keyframes ic-grain {
...
.ic-vig {
  ...
  background: radial-gradient(ellipse 72% 72% at 50% 50%, transparent 0%, rgba(6,8,12,.82) 100%);
  pointer-events: none; z-index: 0;
}
.ic-light {
  position: absolute; top: -20vh; left: 50%; transform: translateX(-50%);
  width: 600px; height: 600px;
  background: radial-gradient(ellipse at center, rgba(184,152,42,.045) 0%, transparent 65%);
  pointer-events: none; z-index: 0;
}

/* ── LAYOUT: three rows top / center / bottom ─ */
.ic-top {
  position: relative; z-index: 5;
  display: flex; justify-content: space-between; align-items: center;
  padding: 22px 48px;
  border-bottom: 1px solid rgba(184,152,42,.12);
  opacity: 0; animation: ic-appear 1s ease forwards 3.6s;
  flex-shrink: 0;
}
...
.ic-eyebrow {
  display: block; font-family:'Cormorant SC',serif; font-size:10px;
  letter-spacing:.55em; color:#B8982A;
  opacity: 0; animation: ic-appear 1.4s ease forwards .5s;
  margin-bottom: 20px;
}
.ic-title {
  ...
  line-height: 1; letter-spacing: -.025em; color: #E8E0CC;
}
.ic-tw { display: block; opacity: 0; transform: translateY(32px); }
...
.ic-rule { width: 32px; height: 1px; background: #B8982A; margin: 20px auto; opacity: 0; animation: ic-appear 1s ease forwards 2.4s; }

.ic-tagline {
  font-family:'Cormorant',serif; font-size: clamp(14px, 1.8vw, 18px);
  font-style: italic; color: rgba(232,224,204,.42);
  letter-spacing:.04em; line-height: 1.7;
  opacity: 0; animation: ic-appear 1.4s ease forwards 2.7s;
  max-width: 400px; margin: 0 auto;
}

/* Gate */
.ic-gate { margin-top: 28px; opacity: 0; animation: ic-appear 1.2s ease forwards 3.1s; }
.ic-access-label { font-family:'Cormorant SC',serif; font-size:8.5px; letter-spacing:.5em; color:rgba(184,152,42,.45); margin-bottom:12px; }
.ic-input-row { display:flex; gap:0; max-width:340px; margin:0 auto; border-bottom:1px solid rgba(184,152,42,.35); padding-bottom:5px; }
.ic-email-input { flex:1; background:transparent; border:none; outline:none; font-family:'Cormorant',serif; font-size:17px; font-style:italic; color:#E8E0CC; padding:3px 0; min-width:0; }
.ic-email-input::placeholder { color:rgba(232,224,204,.28); }
.ic-check-btn { background:none; border:none; cursor:pointer; font-family:'Cormorant SC',serif; font-size:9px; letter-spacing:.35em; color:#B8982A; padding:3px 0 3px 14px; white-space:nowrap; transition:color .3s; }
.ic-check-btn:hover { color:#D4B050; }
.ic-check-btn:disabled { opacity:.4; cursor:default; }
.ic-gate-msg { margin-top:12px; font-family:'Cormorant SC',serif; font-size:9px; letter-spacing:.28em; color:rgba(184,152,42,.55); }

/* Approved state */
.ic-welcome { margin-top: 28px; opacity: 0; animation: ic-appear 1.2s ease forwards 3.1s; }
.ic-edition-link { display:inline-block; font-family:'Cormorant SC',serif; font-size:10px; letter-spacing:.38em; color:#B8982A; text-decoration:none; border-bottom:1px solid rgba(184,152,42,.28); padding-bottom:3px; transition:color .4s,border-color .4s; cursor:pointer; background:none; border-top:none; border-left:none; border-right:none; }
...
.ic-bottom {
  ...
  border-top: 1px solid rgba(184,152,42,.1);
  opacity: 0; animation: ic-appear 1.2s ease forwards 4.0s;
  flex-shrink: 0;
}
.ic-bl-left { font-family:'Cormorant',serif; font-size:12px; font-style:italic; color:rgba(232,224,204,.38); line-height:1.65; }
.ic-bl-right { text-align:right; flex-shrink:0; margin-left:16px; }
.ic-bl-label { font-family:'Cormorant SC',serif; font-size:8px; letter-spacing:.32em; color:rgba(184,152,42,.4); margin-bottom:4px; }
.ic-bl-email { font-family:'Cormorant',serif; font-size:13px; font-style:italic; color:#B8982A; text-decoration:none; transition:color .35s; }
...
@keyframes ic-riseIn { from{opacity:0;transform:translateY(32px)} to{opacity:1;transform:translateY(0)} }

/* ── MOBILE ───────────────────────────────── */
@media (max-width: 768px) {
  .ic-top { padding: 14px 20px; }
  .ic-tb-right { display: none; }
  .ic-middle { padding: 16px 24px; align-items: center; }
  .ic-title { font-size: clamp(44px, 13vw, 72px); line-height: .95; }
  .ic-rule { margin: 14px auto; }
  .ic-tagline { font-size: 15px; max-width: 88vw; }
  .ic-gate { margin-top: 20px; }
  .ic-input-row { max-width: min(340px, 85vw); }
  .ic-bottom { padding: 12px 20px 16px; flex-direction: column; align-items: flex-start; gap: 4px; }
  .ic-bl-right { text-align: left; margin-left: 0; }
  .ic-bl-email { font-size: 12px; }
  .ic-eyebrow { margin-bottom: 14px; }
}
@media (max-width: 420px) {
  .ic-title { font-size: clamp(38px, 15vw, 56px); }
  .ic-tagline { font-size: 14px; }
  .ic-top { padding: 12px 16px; }
  .ic-middle { padding: 12px 16px; }
  .ic-bottom { padding: 10px 16px 14px; }
  .ic-input-row { max-width: 90vw; }
  .ic-check-btn { font-size: 8px; letter-spacing:.25em; }
}
`;
...
export default function Community({ lang = "es" }) {
  ...
  const t = {
    es: {
      ...
      bottomLeft:"Distribuido el primer martes de cada mes. No reenviar.",
    ...
    en: {
      ...
      bottomLeft:"Distributed on the first Tuesday of each month. Not for forwarding.",
  ...
  const checkAccess = async () => {
    ...
    } catch { setGateMsg("Error. Por favor inténtelo de nuevo."); }
  ...
  return (
    <>
      ...
      <div className="ic-root" style={{cursor:"none"}}>
        <div className="ic-grain" />
        <div className="ic-vig"   />
